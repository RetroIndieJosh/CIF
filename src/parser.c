...
void split_tokens(const char * input, int index, int start, int end);
...
void parser_process(const char * input) 
{
        ...
        for (int i = 0; i < strlen(input); ++i) {
                if (input[i] != ' ')
                       continue;
                split_tokens(input, token_count, start, i);
                start = i + 1;
                ++token_count;
        ...
        split_tokens(input, token_count, start, strlen(input) - 1);
        ++token_count;
